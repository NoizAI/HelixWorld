...
class Attention(torch.nn.Module):
    def __init__(
        ...
        # Added post-load by ``LTXModel.enable_video_controls()`` only on each
        # video self-attention module. Keeping this absent from the pretrained
        # state dict preserves strict compatibility with unmodified models.
        self.camera_to_out: torch.nn.Linear | None = None

    def enable_camera_control(self) -> None:
        """Attach the full-rank, zero-initialized PRoPE output projection."""

        if self.camera_to_out is not None:
            return
        reference = self.to_out[0].weight
        self.camera_to_out = torch.nn.Linear(
            self.heads * self.dim_head,
            self.to_out[0].out_features,
            bias=True,
            device=reference.device,
            dtype=reference.dtype,
        )
        torch.nn.init.zeros_(self.camera_to_out.weight)
        torch.nn.init.zeros_(self.camera_to_out.bias)

    @staticmethod
    def _camera_attention_mask(
        attention_mask: torch.Tensor | None,
        camera_control: VideoControlCondition,
        dtype: torch.dtype,
    ) -> torch.Tensor:
        """Combine the base attention bias with invalid camera-key masking."""
        ...
        key_bias.masked_fill_(~key_mask.to(dtype=torch.bool).view(key_mask.shape[0], 1, 1, -1), torch.finfo(dtype).min)
        return key_bias if attention_mask is None else attention_mask + key_bias

    def _camera_attention(
        self,
        q: torch.Tensor,
        k: torch.Tensor,
        v: torch.Tensor,
        ...
        """Run PRoPE attention from pre-RoPE, RMS-normalized shared Q/K/V."""

        if camera_control.camera_projection is None:
            ...
            projection = camera_control.camera_projection
            projection_inverse = camera_control.camera_projection_inverse
        camera_q = apply_tiled_projective_matrix(q, projection.transpose(-1, -2), self.heads)
        camera_k = apply_tiled_projective_matrix(k, projection_inverse, self.heads)
        camera_v = apply_tiled_projective_matrix(v, projection_inverse, self.heads)
        camera_mask = self._camera_attention_mask(attention_mask, camera_control, camera_q.dtype)
        ...
        # Query masking happens after the learned projection so a trained bias
        # cannot leak a residual into reference/invalid tokens.
        camera_out = camera_out * camera_control.camera_valid_mask.to(camera_out.dtype).unsqueeze(-1)
        if perturbation_mask is not None:
            # Unlike the base branch there is no raw-V bypass for camera
            # conditioning: perturbed samples must remove the branch entirely.
    ...
    def forward(
        ...
        Args:
            ...
            camera_control: Optional per-token camera condition. This is only
                valid for video self-attention modules whose camera projection
                has been enabled post-load.
        Returns:
        ...
        if not use_attention:
            ...
            q = self.to_q(x)
            k = self.to_k(context)
            if camera_control is not None and camera_control.has_camera and self.camera_to_out is not None:
                # Ask the configured pre-attention op for RMSNorm-only Q/K.
                # The normal path below still invokes the exact original
                # preattention call (including fused Q8 RMSNorm+RoPE).
