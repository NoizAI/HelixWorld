...
from ltx_core.model.transformer.video_control import ActionTimestepEmbedder, build_projective_matrices
...
class LTXModel(torch.nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self._precompute_camera_projective_matrices = False
        ...
        # Video controls are intentionally attached only after loading the
        # pretrained checkpoint. This keeps the original checkpoint schema and
        # strict base-model loading behavior unchanged.
        self.video_action_embedder: ActionTimestepEmbedder | None = None
        self.video_controls_enabled = False
    ...
    def set_precompute_camera_projective_matrices(self, enable: bool) -> None:
        """Build non-trainable camera matrices once, before transformer blocks."""

        self._precompute_camera_projective_matrices = enable
    ...
    def enable_video_controls(self) -> None:
        """Attach full-rank camera PRoPE and action-conditioning modules.

        Call this after the pretrained LTX checkpoint has been materialized.
        Every video self-attention block receives an independent full
        ``inner_dim -> inner_dim`` camera output projection, and the video
        timestep preprocessor receives one shared action embedder. All newly
        effective output layers are zero-initialized, so
        the first controlled forward is exactly the pretrained model function.
        Repeated calls are idempotent.
        """

        if not self.model_type.is_video_enabled():
            raise RuntimeError("Video controls cannot be enabled on an audio-only LTX model")

        reference = self.patchify_proj.weight
        if reference.device.type == "meta":
            raise RuntimeError("enable_video_controls() must be called after pretrained weights are materialized")

        if self.video_action_embedder is None:
            self.video_action_embedder = ActionTimestepEmbedder(self.inner_dim).to(
                device=reference.device,
                dtype=reference.dtype,
            )

        for block in self.transformer_blocks:
            block.attn1.enable_camera_control()

        video_preprocessor = self.video_args_preprocessor
        if isinstance(video_preprocessor, MultiModalTransformerArgsPreprocessor):
            video_preprocessor = video_preprocessor.simple_preprocessor
        video_preprocessor.action_embedder = self.video_action_embedder
    ...
    def forward(
        ...
        if video is not None and video.video_control is not None and not self.video_controls_enabled:
            raise RuntimeError(
                "video_control was provided before control modules were attached; "
                "load the pretrained checkpoint, then call enable_video_controls()"
            )

        video_args = self.video_args_preprocessor.prepare(video, audio) if video is not None else None
        audio_args = self.audio_args_preprocessor.prepare(audio, video) if audio is not None else None
        if (
            self._precompute_camera_projective_matrices
            and video_args is not None
            and video_args.video_control is not None
            and video_args.video_control.has_camera
        ):
            control = video_args.video_control
            ...
            video_args = replace(
                video_args,
                video_control=replace(
                    control,
                    camera_projection=projection,
                    camera_projection_inverse=projection_inverse,
                ),
            )
