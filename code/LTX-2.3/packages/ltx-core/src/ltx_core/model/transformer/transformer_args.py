...
from ltx_core.model.transformer.video_control import ActionTimestepEmbedder, VideoControlCondition
...
class TransformerArgs:
    ...
    video_control: VideoControlCondition | None = None
...
class TransformerArgsPreprocessor:
    def __init__(  # noqa: PLR0913
        ...
        # Populated by ``LTXModel.enable_video_controls()`` after pretrained
        # weights have been loaded. The module itself remains registered on the
        # model; this preprocessor only holds a convenient reference.
        self.action_embedder: ActionTimestepEmbedder | None = None
    ...
    def _prepare_timestep(
        ...
        if self.action_embedder is None or video_control is None or not video_control.has_action:
            ...
            # Add action after the raw diffusion timestep embedding, before
            # AdaLN's SiLU + linear. The combined embedding is returned as well
            # so the final output AdaLN receives the same action condition.
            embedded_timestep = adaln.emb(timestep_scaled.flatten(), hidden_dtype=hidden_dtype)
            embedded_timestep = embedded_timestep.view(batch_size, -1, embedded_timestep.shape[-1])
            action_embedding = self.action_embedder(
                video_control.action_ids,
                video_control.action_valid_mask,
                hidden_dtype=hidden_dtype,
            )
            embedded_timestep = embedded_timestep + action_embedding
            timestep = adaln.linear(adaln.silu(embedded_timestep)).flatten(0, 1)
    ...
    def prepare(
        ...
        if modality.video_control is not None:
            modality.video_control.validate(batch_size=modality.latent.shape[0], num_tokens=modality.latent.shape[1])
